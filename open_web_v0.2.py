...
# when a window is called by the main page handle_request is triggered. It filters the url,
# and if it is containing /ffs/logs/ and it ens with /print the user is trying to print a 
# techlog. It detects the save window, adds the name of the tech
async def handle_request(context, request):
    ...
    if "/ffs/logs/" in url and url.endswith("/print"):
        ...
        while time.time() - start_time < 10:
            try:
                windows = findwindows.find_windows(title_re=".*Guardar impresión como.*|.*Save As.*|.*Save Print Output As.*")
                if windows:
        ...
        for _ in range(10):
            ...
            if current_text == str(filename):
                dlg['Save|&SaveButton'].click()  # botón Guardar
